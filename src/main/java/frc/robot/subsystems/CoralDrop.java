...
import frc.robot.Constants.RampCostants;
...
public class CoralDrop extends SubsystemBase{
    ...
    public CoralDrop(){
        coralMotor = new SparkMax(RampCostants.CoralDropMotorID, MotorType.kBrushless);
    ...
    public void setSpeed(){
        if(isRunning) {
       closedLoopController.setReference(RampCostants.CoralDropSpeed, ControlType.kVelocity, ClosedLoopSlot.kSlot1);
        }
        else{
            closedLoopController.setReference(0, ControlType.kVelocity, ClosedLoopSlot.kSlot1);
        }
    }
